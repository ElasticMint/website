...
1. **User roles.** One kind of user is cheap. Every additional role, such as admin, manager, customer or approver, adds screens, permissions and logic to build and to test.
2. **Integrations.** Every external system you connect to is a small project of its own: payment gateways, accounting tools, mapping, push notifications, single sign-on.
3. **Data complexity.** Simple records are quick. Relationships between records, reporting and data that has to stay accurate under load all cost more.
